...
for epoch in range(epochs):  # loop over the dataset multiple times
    ...
    #     training_loss / (epoch + 1)), val_loss="{:.8f}".format(validation_loss))

    logger.add_scalar(f'{name}/train_loss', loss_train.item(), epoch*len(dataloader_train)+epoch)
    logger.add_scalar(f'{name}/train_accuracy', train_accuracy, epoch*len(dataloader_train)+epoch)
    ...
    with torch.no_grad():
        ...
        logger.add_scalar(f'{name}/val_loss', validation_loss/(
            epoch+1), epoch*len(dataloader_validation)+epoch)
