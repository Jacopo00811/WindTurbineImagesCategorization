...
from MultiModel import MultiModel, FineTuneMode
...
def train_net(model, loss_function, device, dataloader_train, dataloader_validation, optimizer, hyper_parameters, logger, scheduler, state, name="default"):
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for train_iteration, batch in training_loop:
            ...
            logger.add_scalar(f'{name}/Train loss_{state}', loss_train.item(), epoch*len(dataloader_train)+train_iteration)
            logger.add_scalar(f'{name}/Train accuracy_{state}', train_accuracy, epoch*len(dataloader_train)+train_iteration)
